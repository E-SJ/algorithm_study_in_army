### 22/11/10
* 백준-9527(c5) 완료 - 정말 어거지로 규칙을 찾아서 성공시켰다. (이게 모범답안인지는 모르겠지만..) 다만 처음에 올바른 풀이였음에도 오답이 나왔는데, 분석해보니 log 함수의 출력 값이 ~.000000004 같은 식으로 약간의 오차가 발생했고 이 값이 10^16 가량의 큰 값에는 이 오차가 유효하게 작용해 틀린 답을 내놓았었던 것이다. 따라서 log 출력 값에서부터 int를 씌워 오차의 발생 여지를 차단하니 정상적인 답이 나와 통과할 수 있었다.(class 5 달성으로 G3에서 G1으로 승급했다!)
### 22/11/09
