...
if (location[-1]>0) : #가장 먼거리에 있는 책이 양수위치일때
  if (len(n_location)%m-1>=0):
    result+=abs(n_location[len(n_location)%m-1])*2
  for _ in range(len(n_location)%m):
    n_location.pop(0)
    
  while len(n_location)!=0:  
    result+=abs(n_location[m-1])*2
    for _ in range(m):
      n_location.pop(0)

  if (len(p_location)%m-1>=0 and len(p_location)//m>=1):
    result+=p_location[len(p_location)%m-1]*2
  for _ in range(len(p_location)%m and len(p_location)//m>=1):
    p_location.pop(0)

  while len(p_location)>m:
  ...
  if (len(p_location)%m-1>=0):
    result+=p_location[len(p_location)%m-1]*2
  for _ in range(len(p_location)%m-1):
    p_location.pop(0)

  while len(p_location)!=0:  
    result+=p_location[m-1]*2
    for _ in range(m):
      p_location.pop(0)
      
  if (len(n_location)%m-1>=0 and len(n_location)//m>=1):
    result+=abs(n_location[len(n_location)%m-1])*2
  for _ in range(len(n_location)%m-1 and len(n_location)//m>=1):
    n_location.pop(0)
    
  while len(n_location)>m:
    result+=abs(n_location[m-1])*2
    for _ in range(m):
      n_location.pop(0)
