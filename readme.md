### 22/11/25
* [연등] 백준-2887(c5) 완료 - 한동안 머신러닝 공부하다가 간만에 백준을 다시 풀어보았다. 최소 스패닝 트리 문제였는데, 간만에 크루스칼 알고리즘으로 구현해보았는데도 생각보다 구현에서 막힘은 크게 없었지만, 모든 간선을 구하려고 하니 메모리 초과라는 벽에 부딪혔다. 따라서 최소 스패닝 트리에 쓰일만한 간선을 일부만 추려서 큐에 추가시켜야 했는데 그 아이디어를 떠올리지 못해서 그 부분만 도움을 받았다. [(아이디어 참고)](https://dev-mb.tistory.com/249)
### 22/11/13
